  ...
  pub fn get_result_exception(&self) -> Result<(), PyError> {
    unsafe {
      ...
      if pvalue.is_null() {
        ...
        let error_type_string = self.PyObject_GetAttrString(ptype, CString::new("__name__").unwrap().as_ptr());
...
impl<'a> PyObject<'a> {
  ...
  pub fn get_member_obj(&self, name: &str) -> Result<PyObject<'a>, PyError> {
    unsafe {
      let py_member = self.state.PyObject_GetAttrString(self.raw, CString::new(name).unwrap().as_ptr());
  ...
  pub fn call_func<'b, I : ToPyType>(&'b self, name : &str, args : I) -> Result<PyObject<'b>, PyError> {
  ...
  pub fn call_func_with_ret<I : ToPyType, R : FromPyType>(&self, name : &str, args : I) -> Result<R, PyError> {
    self.get_func(name).and_then(|x| {
      args.to_py_object(self.state).and_then(|input| {
...
pub struct PyIterator<'a, T> { py_object : PyObject<'a>, _phantom: PhantomData<T> }
  ...
  pub fn new(obj : PyObject<'a>) -> Result<PyIterator<'a, T>, PyError> {
    unsafe {
      if obj.state.PyIter_Check(obj.raw) != 0 {
        Ok(PyIterator { py_object : obj, _phantom: PhantomData })
      } else {
...
mod test {
  ...
  fn test_exceptions_module() {
    ...
    match module {
      ...
      Err(PyError::PyException(s)) => assert_eq!(&s, "ImportError : No module named mathSpelledWrong"),
  ...
  fn test_exceptions_function_lookup() {
    ...
    match func {
      Ok(_) => panic!("Did not return Err"),
      Err(PyError::PyException(s)) => assert_eq!(&s, "AttributeError : 'module' object has no attribute 'powMissSpelled'"),
      Err(e) => panic!("Got unexpected error: {:?}", e)
    };
  }
  ...
  fn test_exceptions_function_call() {
    ...
    match res {
      Ok(_) => panic!("Did not return Err"),
      Err(PyError::PyException(s)) => assert_eq!(&s, "TypeError : pow expected 2 arguments, got 3"),
      Err(e) => panic!("Got unexpected error: {:?}", e)
    };
  }
  ...
  fn test_py_object_show() {
    let py = PyState::new();
    let tup = (1, 2f32);
    assert_eq!(format!("{:?}", tup.to_py_object(&py).unwrap()), "PyObject{(1, 2.0)}".to_string());
  }
  ...
  fn iterate_combinations() {
    ...
    let result = try_or_panic!(comb.iter()).map(|x| x.unwrap()).collect::<Vec<(isize, isize)>>();
